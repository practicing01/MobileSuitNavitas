...
void Splash::HandleUpdate(StringHash eventType, VariantMap& eventData)
{
	...
	if (elapsedTime_ >= 2.0f)
	{
		...
		main_->logicStates_.Push(new SimulationMenu(context_, main_));
		delete this;
	}
}
...
void Splash::SplashScreen()
{
	...
	Node* bgm = main_->scene_->CreateChild("bgm");
	bgm->AddComponent(new SoundSource(main_->GetContext()), 0, LOCAL);
	bgm->GetComponent<SoundSource>()->SetSoundType(SOUND_MUSIC);
	bgm->GetComponent<SoundSource>()->Play(main_->cache_->GetResource<Sound>("Music/misc_sound.ogg"));
}
